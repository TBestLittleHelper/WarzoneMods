...
function First(array, func)
    for _, v in pairs(array) do if (func(v)) then return v end end
    return nil
end
...
function StartsWith(str, sub) return string.sub(str, 1, string.len(sub)) == sub end
...
function AddToSet(set, key) set[key] = true end

function RemoveFromSet(set, key) set[key] = nil end
